...
const Navbar = () => {
  ...
  // --- Added "Archive" for Gallery3D ---
  const menuItems = [
    { name: "Home", id: "home", num: "01" },
    { name: "About", id: "about", num: "02" },
    { name: "Gallery", id: "gallery", num: "03" },
    { name: "The Archive", id: "gallery3d", num: "04" }, // Yeh Gallery3D ke liye hai
    {
      name: "Projects",
      id: "projects",
      num: "05",
    ...
    { name: "Contact Us", id: "contact", num: "06" },
  ];
  ...
  return (
    <>
      <style dangerouslySetInnerHTML={{ __html: `
        @import url('https://fonts.googleapis.com/css2?family=Cinzel:wght@400;700&family=Montserrat:wght@200;300;400;500;600&display=swap');
      ...
      >
        <nav className="max-w-[1500px] mx-auto w-full flex items-center px-10">
          ...
          <div className="hidden lg:flex flex-1 justify-center gap-10 font-luxury items-center">
            {menuItems.map((item, index) => (
              <div key={index} className="relative flex flex-col items-center group">
                <button
                  onClick={() => scrollToSection(item.id)}
                  className={`text-[9px] font-bold uppercase text-spacing transition-all duration-300 flex items-center gap-1 pb-1 ${
          ...
          <div className="hidden lg:flex items-center">
            <motion.button 
              ...
              className="px-6 py-2.5 bg-[#B8860B] text-white text-[10px] font-bold uppercase tracking-[0.2em] rounded-full glow-button flex items-center gap-2"
            >
            ...
            <button onClick={() => setIsOpen(!isOpen)} className="relative w-10 h-10 flex items-center justify-center focus:outline-none z-[400]">
              <div className="flex flex-col items-center justify-center relative w-6 h-6">
                <motion.span 
                  ...
                  className="absolute w-6 h-[2px] bg-[#B8860B] block rounded-full"
              ...
              {menuItems.map((item, index) => (
                <div key={index} className="overflow-hidden">
                  ...
                  >
                    ...
                    <span className={`text-2xl uppercase font-medium tracking-[0.1em] ${active === item.id ? "text-[#B8860B]" : "text-gray-400"}`}>
                      {item.name}
                    </span>
                    {item.dropdown && (
                      <motion.div animate={{ rotate: dropdown === index ? 90 : 0 }} className="ml-auto">
                        <FiChevronRight size={20} className={dropdown === index ? 'text-[#B8860B]' : 'text-gray-300'} />
                      </motion.div>
                    )}
                  </motion.div>
                  ...
                  <AnimatePresence>
                    {dropdown === index && item.dropdown && (
                      <motion.div
                        ...
                        className="overflow-hidden ml-8 mt-4 space-y-4 border-l-2 border-[#B8860B]/10 pl-6"
                      >
                        {item.dropdown.map((sub, i) => (
                          <button key={i} onClick={() => scrollToSection(sub.id)} className="block text-[13px] font-medium uppercase text-spacing text-gray-400 text-left w-full py-1">
                            {sub.name}
                          </button>
                        ))}
                      </motion.div>
